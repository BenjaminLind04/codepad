...
function codepad_share() {
    codepad_create_share_url()
    var share_options = document.getElementById("share_options");

    share_options.style.display = "block";

    var span = document.getElementsByClassName("close")[0];
    var p = document.getElementById("url")
    p.innerHTML = window.location

    span.onclick = function () {
        share_options.style.display = "none";
    }

    window.onclick = function (event) {
        if (event.target == share_options) {
            share_options.style.display = "none";
        }
    }
}
...
function copy_to_clipboard() {
    ...
    test.innerHTML = "Copied to clipboard!"
}

function save_as_txt() {
    var project_name = project_info.title + "_link"
    const saveDate = new Date();


    var content = new Blob([`Project name: ${project_name}. \n \n
    link: ${window.location}
    Save date ${saveDate}
    
    How to use: Copy this link and paste it in a browser to run the saved project`], {
        type: "text/plain;charset=utf-8",
    });

    saveAs(content, project_name)

    console.log(project_info)
}

function codepad_create_share_url() {
